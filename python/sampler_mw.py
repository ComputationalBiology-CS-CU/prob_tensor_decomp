...
def data_prepare():
	...
	#dataset = np.zeros(shape=(n_individual,n_gene,n_tissue))
	#markerset = np.zeros(shape=(n_individual,n_gene,n_tissue))
	...
	# === fill the simulated data into tensor ===
...
def sampler_MVN(mean, cov):
	...
	return x  # return a np array of shape (n_pts, len(mean))
	'''
...
def sampler_factor(factor_id):
	...
	for i in range(cur_dimension):	# there are n factor array, that are independent with each other --> parallel
		...
		#markerset[(1,2,3)] <=> markerset[1][2][3]
	...
	for count1 in range(n_factor):
		for count2 in range(n_factor):
			cov_matrix[count1][count2] += temp * (hyper_prior[factor_id][2] - factor_mean[count1]) * (hyper_prior[factor_id][2] - factor_mean[count2])
...
def loglike_MVN(obs, mean, precision):
	...
	# distance term
	array1 = map(lambda x1,x2: np.add(x1, -1*x2), obs, mean)
	array2 = []
	for i in range(n_factor):
		value = 0
		for j in range(n_factor):
...
def loglike_joint():
	...
	for k in range(n_tissue):
		like_log += loglike_MVN(fmlist[2][k], prior[2][0], prior[2][1])
...
if __name__ == '__main__':
	...
	fmlist.append(sampler_MVN(mean, cov, n_tissue))
	#dimension 2: individual    n_individual x n_factor
	fmlist.append(sampler_MVN(mean, cov, n_individual))
	#dimension 3: gene    n_gene x n_factor
	fmlist.append(sampler_MVN(mean, cov, n_gene))
	'''
